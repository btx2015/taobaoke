...
class SettlementController extends CommonController
{
    ...
    public function index(){
        if(IS_POST){
            ...
            returnResult([
                'list' => handleRecords([
                    'channel_percent' => ['percent',2,'channel_percent_str'],
                    'member_percent'  => ['percent',2,'member_percent_str'],
    ...
    public function detail(){
        if(IS_POST){
            list($where,$pageNo,$pageSize) = before_query([
                'page'        => [['num'],1],
                'rows'        => [['num'],10],
                'id'          => [['num'],false,true,['settlement_id']],
                'type'        => [['in' => [1,2]],false,true],
                'create_from' => [['time'],false,true,['egt','created_at']],
                'create_to'   => [['time'],false,true,['elt','created_at']],
            ]);

            $list = M(self::T_SETTLE)->where($where)->page($pageNo,$pageSize)->select();
